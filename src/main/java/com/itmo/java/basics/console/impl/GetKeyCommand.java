...
public class GetKeyCommand implements DatabaseCommand {
    ...
    public GetKeyCommand(ExecutionEnvironment env, List<RespObject> commandArgs) {
        ...
        if (commandArgs.size() != numberOfAgrguments) {
            throw new IllegalArgumentException(String.format("Wrong number of arguments - you need %d but given %d", numberOfAgrguments, commandArgs.size()));
        }
    }
    ...
    public DatabaseCommandResult execute() {
        try {
            ...
            if (value.isEmpty()) {
                throw new DatabaseException(String.format("Value for key %s in table %s in database %s is absent", key, tbName, tbName));
            }
